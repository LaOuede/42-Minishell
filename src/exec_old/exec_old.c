...
void	ft_make_pids_old(t_exec *exec)
{
	int	i;
...
char	*ft_cmd_path_old(t_exec *exec, char *cmds)
{
	char	*path;
	int		i;
...
void	ft_run_cmd_old(t_exec *exec)
{
	char	*path;
	// char	*cmds[1] = {"ls"};
	char	**cmds;
...
void	ft_dup_process_old(t_exec *exec, int i)
{
	exec->index = i;
	if (exec->index == 0)
	{
